"""
This performs mesh refinement experiment on the JB problem using the EVSS formulation 
of the Oldroyd 3 model. We assume no knowledge of the exact solution, and the convergence
...
To change to a different solver, make sure you change the output file name on line
38(42) and when you call the solver on line 79(83)
"""
...
h_array = -1.0*np.array([0, 1, 2, 3, 4]) # 4th mesh refinement, FEniCS killed the process
h_array = 0.2 * (2 ** h_array)

# For saving the info
############# CHANGE THIS DIFFERENT METHODS ##############
table_file = open('results_jb_' + model + '/mesh_ref_jb_Wi=%.3e_s=%.3e_l1=%.3e_'%(Wi, speed, lambda1) + model + '_evss.csv', 'w') 
